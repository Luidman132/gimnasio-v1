<?php
require_once 'conexion.php';

$usuarioActual = requerir_sesion($conexion, soloAdmin: true);

solo_metodo('POST');
$data = leer_json();

$nombre = trim((string) campo($data, 'nombre', ''));
$precio = campo($data, 'precio');
$duracion = campo($data, 'duracion_dias');

if ($nombre === '' || !is_numeric($precio) || !is_numeric($duracion)) {
    responder_error('Faltan datos obligatorios del plan.', 400);
}
if ((float) $precio < 0 || (int) $duracion <= 0) {
    responder_error('El precio o la duración del plan no son válidos.', 400);
}

try {
    $sql = 'INSERT INTO planes
                (nombre, precio, duracion_dias, es_promocion,
                 fecha_inicio_venta, fecha_fin_venta, nota, activo)
            VALUES
                (:nombre, :precio, :duracion, :es_promo,
                 :fecha_inicio, :fecha_fin, :nota, 1)';

    $conexion->prepare($sql)->execute([
        ':nombre' => $nombre,
        ':precio' => (float) $precio,
        ':duracion' => (int) $duracion,
        ':es_promo' => !empty($data->es_promocion) ? 1 : 0,
        ':fecha_inicio' => campo($data, 'fecha_inicio_venta'),
        ':fecha_fin' => campo($data, 'fecha_fin_venta'),
        ':nota' => campo($data, 'nota'),
    ]);

    responder(['success' => true, 'mensaje' => '¡Plan guardado con éxito!']);
} catch (PDOException $e) {
    responder_error('Error al guardar el plan.', 500, $e);
}
